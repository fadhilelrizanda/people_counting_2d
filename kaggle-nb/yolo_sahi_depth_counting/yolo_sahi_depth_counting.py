...
def install_deps():
    deps = ["ultralytics", "transformers", "sahi", "supervision"]
    for dep in deps:
...
def load_detection_model(device_str):
    for model_name in MODEL_CANDIDATES:
        try:
            # We explicitly load YOLO once to trigger auto-download if needed
            YOLO(model_name)
...
def process_video(input_path, output_path, tracker_type, rank):
    ...
    print(f"[Rank {rank}] Processing {input_path} ({max_duration_sec} seconds) with optimized Depth-Anything V2 BEV + SAHI...")
    ...
    while cap.isOpened() and frame_count < max_frames:
        ...
        tracked_detections = sv_tracker.update_with_detections(detections)
